...
public class Pemasukan : MonoBehaviour
{
    ...
    public void actionSimpan(){
        if(string.IsNullOrEmpty(tanggal.text) || string.IsNullOrEmpty(nominal.text) || string.IsNullOrEmpty(keterangan.text) ){
            ...
            string[] pemasukanInsert = new string[4] {"'"+tanggal.text+"'","'"+nominal.text+"'","'"+keterangan.text+"'","'pemasukan'"};
			int resultPemasukanInsert = db.InsertInto("details", pemasukanInsert);
			if(resultPemasukanInsert>0){
    ...
    IEnumerator Notification(int timer, string tipe)
    {
        if(tipe=="sukses"){
            ...
            textNotification.text= "Sukses menambahkan pemasukan baru" ;
            notification.SetActive(true);
            yield return new WaitForSeconds(timer);
            actionKembali();
        }else if(tipe=="gagal"){
            notification.transform.GetChild(0).GetComponent<Image>().color= red;
            textNotification.text= "Gagal menambahkan pemasukan baru" ;
            notification.SetActive(true);
            yield return new WaitForSeconds(timer);
            notification.SetActive(false);
        }else{
